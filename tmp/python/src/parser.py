...
class Parser:
    ...
    def eger_blogu(self):
        ...
        # Komutları oku. Ta ki 'EĞER' (blok sonu için) veya 'DEĞİLSE' görene kadar.
        while self.mevcut_token.tip not in [
            TOKEN_TIPLERI['YAPI_KOSUL_EGER'],  # 'EĞER SON' formatı için
        ...
        if self.mevcut_token.tip == TOKEN_TIPLERI['YAPI_KOSUL_DEGILSE']:
            ...
            # 'EĞER' (blok sonu için) görene kadar komutları oku
            while self.mevcut_token.tip not in [TOKEN_TIPLERI['YAPI_KOSUL_EGER'], TOKEN_TIPLERI['EOF']]:
                yanlis_blok.komutlar.append(self.komut())

        # 'EĞER SON' formatı
        self.tuket(TOKEN_TIPLERI['YAPI_KOSUL_EGER'])  # 'EĞER' kelimesini tüket
        self.tuket(TOKEN_TIPLERI['YAPI_SON'])  # 'SON' kelimesini tüket
    ...
    def dongu_blogu(self):
        """
        DÖNGÜ komutlar DÖNGÜ SON
        veya
        DÖNGÜ komutlar SON DÖNGÜ
        """
        self.tuket(TOKEN_TIPLERI['YAPI_DONGU']) # 'DÖNGÜ'

        # Döngü Bloğu: 'DÖNGÜ' (blok sonu için) veya 'SON' görene kadar komutları oku
        blok = ast_nodes.Blok()
        while self.mevcut_token.tip not in [TOKEN_TIPLERI['YAPI_DONGU'], TOKEN_TIPLERI['YAPI_SON'], TOKEN_TIPLERI['EOF']]:
            blok.komutlar.append(self.komut())

        # DÖNGÜ bloğu 'DÖNGÜ SON' veya 'SON DÖNGÜ' ile bitmeli
        if self.mevcut_token.tip == TOKEN_TIPLERI['YAPI_SON']:
            # 'SON DÖNGÜ' formatı
            self.tuket(TOKEN_TIPLERI['YAPI_SON'])
            self.tuket(TOKEN_TIPLERI['YAPI_DONGU'])  # 'DÖNGÜ' kelimesini bekle
        else:
            # 'DÖNGÜ SON' formatı
            self.tuket(TOKEN_TIPLERI['YAPI_DONGU'])
            self.tuket(TOKEN_TIPLERI['YAPI_SON'])
